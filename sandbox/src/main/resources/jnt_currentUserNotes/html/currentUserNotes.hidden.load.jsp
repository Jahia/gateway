...
<query:definition var="result"
             statement="select * from [jnt:privateNote] as m  where isdescendantnode(m, ['${user.path}/contents']) order by m.[jcr:lastModified] desc"/>
<c:set target="${moduleMap}" property="listQuery" value="${result}"/>
<template:addCacheDependency flushOnPathMatchingRegexp="\Q${user.path}\E/contents"/>
